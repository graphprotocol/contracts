...
contract DisputeManagerIndexingFeeCreateDisputeTest is SubgraphServiceIndexingAgreementSharedTest {
    ...
    function test_IndexingFee_Create_Dispute_EmitsEvent(Seed memory seed, uint256 unboundedTokensCollected) public {
        ...
        token.approve(address(disputeManager), deposit);

        // forge-lint: disable-next-line(unsafe-typecast)
        bytes32 poi = bytes32("disputePOI");
        uint256 entities = 200;
        uint256 blockNumber = block.number;

        bytes32 expectedDisputeId = keccak256(
            abi.encodePacked("IndexingFeeDisputeWithAgreement", agreementId, poi, entities, blockNumber)
        );

        vm.expectEmit(address(disputeManager));
        emit IDisputeManager.IndexingFeeDisputeCreated(
            expectedDisputeId,
            indexerState.addr,
            users.fisherman,
            deposit,
            ...
            poi,
            entities,
            indexerState.tokens // stakeSnapshot
        );

        bytes32 disputeId = disputeManager.createIndexingFeeDisputeV1(agreementId, poi, entities, blockNumber);
