...
contract DisputeManager is DisputeManagerV1Storage, IDisputeManager {
        ...
        onlyPendingDispute(_disputeID)
    {
        ...
        console.log("Slashing %s tokens from %s", _slashAmount, dispute.serviceProvider);
        console.log("Rewarding %s tokens to %s", tokensToReward, dispute.fisherman);
        console.log("Returning %s tokens to %s", dispute.deposit, dispute.fisherman);
    ...
    function _pullSubmitterDeposit(uint256 _deposit) private {
        ...
        console.log("Pulling %s tokens from %s", _deposit, msg.sender);
