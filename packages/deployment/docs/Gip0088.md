...
| Existing-contract config     | `RC.setPauseGuardian`, `RM.setDefaultReclaimAddress`, `RM.setRevertOnIneligible` (driven by `RewardsManager.revertOnIneligible` config)                 |
...
| Script                  | `--tags`                         | What it does                                                                                                                                                                                    |
| ----------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eligibility_integrate` | `GIP-0088:eligibility-integrate` | `<target>.setProviderEligibilityOracle(<configured oracle>)` for RM and RAM, each from config (`<target>.eligibilityOracle`, a full oracle contract name; omit a target's field for none)       |
...
| `issuance_allocate`     | `GIP-0088:issuance-allocate`     | `IA.setTargetAllocation(target, ...)` for each target in config `IssuanceAllocator.allocations`; errors unless the config total equals RM's on-chain `issuancePerBlock` and the rates sum to it |

**Optional goals** — not planned for initial deployment:

| Script                 | `--tags`                        | What it does                                                                                                                                                                                                               |
| ---------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issuance_close_guard` | `GIP-0088:issuance-close-guard` | `SS.setBlockClosingAllocationWithActiveAgreement(<configured>)` — value from `SubgraphService.blockClosingAllocationWithActiveAgreement` in `config/<network>.json5` (default `true`); skips when on-chain already matches |

(`RM.setRevertOnIneligible` is bundled into `04_upgrade` driven by `RewardsManager.revertOnIneligible` in config — see the upgrade-batch table above.)
...
- `--tags GIP-0088:issuance-allocate` — apply the configured issuance allocation table (governance TX)
...
| Existing-contract config     | `RC.setPauseGuardian(pauseGuardian)`; `RM.setDefaultReclaimAddress(reclaim)` and `RM.setRevertOnIneligible(<configured>)` (only after RM upgrade — bundle order means RM upgrade executes first in the same batch). `setRevertOnIneligible` arg is driven by `RewardsManager.revertOnIneligible` in `config/<network>.json5` (default `true`). |
...
| ----------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eligibility-integrate` | RM/RAM upgraded; each target's configured oracle deployed       | `<target>.setProviderEligibilityOracle(<configured oracle>)` for RM and RAM, each from config (`<target>.eligibilityOracle`, a full oracle contract name; omit for none). Per target, re-points to the configured oracle when it differs (overriding any prior value); no-op when it already matches.                       |
...
| `issuance-allocate`     | IA deployed, configured targets deployed, issuance-connect done | `IA.setTargetAllocation(target, ...)` for each target in config `IssuanceAllocator.allocations`, set to exactly its configured rate (no rebalancing). Errors unless the config total matches RM's on-chain `issuancePerBlock` and the rates sum to it. Skips when no allocations are configured.                            |

#### Optional goals

| Goal                   | Precondition | Notes                                                                                                                                                                                                              |
| ---------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `issuance-close-guard` | SS upgraded  | `SS.setBlockClosingAllocationWithActiveAgreement(<configured>)` from `SubgraphService.blockClosingAllocationWithActiveAgreement` (default `true`); skipped when the on-chain value already matches the desired one |
...
- [Gip0088Runbook.md](Gip0088Runbook.md) — operational runbook: the staged, gated execution plan for the rollout
