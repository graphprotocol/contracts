...
| ----------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eligibility-integrate` | RM/RAM upgraded; each target's configured oracle deployed       | `<target>.setProviderEligibilityOracle(<configured oracle>)` for RM and RAM, each from config (`<target>.eligibilityOracle`, a full oracle contract name; omit for none). Per target, skips if an oracle is already set (does not override).                                                                                |
