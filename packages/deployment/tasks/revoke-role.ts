...
const action: NewTaskActionFunction<TaskArgs> = async (taskArgs, hre) => {
  ...
  if (canExecuteDirectly && walletClient && deployer) {
    ...
    console.log(`\n   Requires ${adminInfo.adminRoleName ?? 'admin role'} to revoke`)
    ...
    const txName = `revoke-${roleName}-from-${targetAccount.slice(0, 8)}`
    const builder = await createGovernanceTxBuilder(env as Parameters<typeof createGovernanceTxBuilder>[0], txName, {
      name: `Revoke ${roleName}`,
      description: `Revoke ${roleName} from ${targetAccount} on ${contractName ?? contractAddress}`,
    })

    // Encode the revokeRole call
