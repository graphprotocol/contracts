...
contract RecurringCollectorHashRoundTripTest is RecurringCollectorSharedTest {
    ...
    /// @notice Stored RCA offer is cleaned up after acceptance
    function test_HashRoundTrip_RCA_CleanedUpAfterAccept() public {
        IRecurringCollector.RecurringCollectionAgreement memory rca = _makeRCA();
        bytes32 rcaHash = _recurringCollector.hashRCA(rca);
        ...
        // Stored offer should be cleaned up after accept
        (, bytes memory offerData) = _recurringCollector.getAgreementOfferAt(agreementId, 0);
        assertEq(offerData.length, 0, "RCA offer should be cleaned up after accept");
    }
    ...
    // ==================== RCAU round-trip (accepted → cleaned up) ====================

    function test_HashRoundTrip_RCAU_CleanedUpAfterUpdate() public {
        IRecurringCollector.RecurringCollectionAgreement memory rca = _makeRCA();
        bytes16 agreementId = _offerAndAcceptRCA(rca);
        ...
        // Stored update offer should be cleaned up
        (, bytes memory offerData) = _recurringCollector.getAgreementOfferAt(agreementId, 1);
        assertEq(offerData.length, 0, "RCAU offer should be cleaned up after update");
    }
    ...
    function test_HashRoundTrip_CancelPending_ActiveStays() public {
        ...
        // RCA offer was already cleaned up at accept time
        (, bytes memory rcaData) = _recurringCollector.getAgreementOfferAt(agreementId, 0);
        assertEq(rcaData.length, 0, "RCA offer should have been cleaned up at accept");
