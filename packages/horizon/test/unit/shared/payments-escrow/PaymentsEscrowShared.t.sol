...
abstract contract PaymentsEscrowSharedTest is GraphBaseTest {
    ...
    function _depositTokens(address _collector, address _receiver, uint256 _tokens) internal {
        ...
        uint256 escrowBalanceBefore = escrow.escrowAccounts(msgSender, _collector, _receiver).balance;
        ...
        uint256 escrowBalanceAfter = escrow.escrowAccounts(msgSender, _collector, _receiver).balance;
    ...
    function _depositToTokens(address _payer, address _collector, address _receiver, uint256 _tokens) internal {
        uint256 escrowBalanceBefore = escrow.escrowAccounts(_payer, _collector, _receiver).balance;
        token.approve(address(escrow), _tokens);
        ...
        uint256 escrowBalanceAfter = escrow.escrowAccounts(_payer, _collector, _receiver).balance;
        assertEq(escrowBalanceAfter - _tokens, escrowBalanceBefore);
    }
}
