...
contract AuthorizableTest is Test, Bounder {
    ...
    modifier withFuzzyThaw(uint256 _thawPeriod) {
        ...
        setupAuthorizable(new Authorizable(_thawPeriod));
        _;
    }

    function setUp() public virtual {
        setupAuthorizable(new Authorizable(0));
    }

    function setupAuthorizable(IAuthorizable _authorizable) internal {
    ...
    function test_AuthorizeSigner_Revert_WhenAlreadyAuthorized(
        ...
        bytes memory expectedErr = abi.encodeWithSelector(
            IAuthorizable.SignerAlreadyAuthorized.selector,
            validAuthorizer,
            signer,
            false
        );
    ...
    function test_AuthorizeSigner_Revert_WhenInvalidProofDeadline(uint256 _proofDeadline, uint256 _now) public {
        ...
        bytes memory expectedErr = abi.encodeWithSelector(
            IAuthorizable.InvalidSignerProofDeadline.selector,
            _proofDeadline,
            _now
        );
        vm.expectRevert(expectedErr);
    ...
    function test_AuthorizeSigner_Revert_WhenInvalidSignerProof(
        uint256 _now,
        ...
        for (uint256 i = 0; i < proofs.length; i++) {
            vm.expectRevert(IAuthorizable.InvalidSignerProof.selector);
            vm.prank(authorizer);
    ...
    function test_ThawSigner_Revert_WhenNotAuthorized(address _authorizer, address _signer) public {
        ...
        bytes memory expectedErr = abi.encodeWithSelector(
            IAuthorizable.SignerNotAuthorized.selector,
            _authorizer,
            _signer
        );
        vm.expectRevert(expectedErr);
    ...
    function test_CancelThawSigner(
        ...
        emit IAuthorizable.SignerThawCanceled(_authorizer, signer, 0);
        vm.prank(_authorizer);
    ...
    function test_CancelThawSigner_Revert_When_NotThawing(address _authorizer, uint256 _unboundedKey) public {
        ...
        bytes memory expectedErr = abi.encodeWithSelector(IAuthorizable.SignerNotThawing.selector, signer);
        vm.expectRevert(expectedErr);
        vm.prank(_authorizer);
        authorizable.cancelThawSigner(signer);
    }
    ...
    function test_RevokeAuthorizedSigner_Revert_WhenStillThawing(
        ...
        _skip = bound(_skip, 0, authorizable.getRevokeAuthorizationThawingPeriod() - 1);
        skip(_skip);
        bytes memory expectedErr = abi.encodeWithSelector(
            IAuthorizable.SignerStillThawing.selector,
            block.timestamp,
            block.timestamp - _skip + authorizable.getRevokeAuthorizationThawingPeriod()
        );
        vm.expectRevert(expectedErr);
        vm.prank(_authorizer);
        authorizable.revokeAuthorizedSigner(signer);
...
contract AuthorizableHelper is Test {
    ...
    constructor(IAuthorizable _authorizable) {
        authorizable = _authorizable;
    }
    ...
    function authorizeAndThawSignerWithChecks(address _authorizer, uint256 _signerKey) public {
        ...
        uint256 thawEndTimestamp = block.timestamp + authorizable.getRevokeAuthorizationThawingPeriod();
        vm.expectEmit(address(authorizable));
    ...
    function authorizeAndRevokeSignerWithChecks(address _authorizer, uint256 _signerKey) public {
        ...
        skip(authorizable.getRevokeAuthorizationThawingPeriod() + 1);
        vm.expectEmit(address(authorizable));
