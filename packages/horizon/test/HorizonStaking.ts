...
describe('HorizonStaking', function () {
  async function deployFixture() {
    ...
    const HorizonStakingExtension = await ethers.getContractFactory('HorizonStakingExtension', { libraries: { ExponentialRebates: exponentialRebates.target } })
    const horizonStakingExtension = await HorizonStakingExtension.deploy(controller.target, ZeroAddress)
