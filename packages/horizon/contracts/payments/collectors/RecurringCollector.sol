    ...
    IRecurringCollector
{
    ...
    function _getMaxNextClaimScoped(bytes16 agreementId, uint8 agreementScope) private view returns (uint256 maxClaim) {
        ...
        if (agreementScope & SCOPE_ACTIVE != 0 && _a.activeTermsHash != bytes32(0)) {
            ...
            // Pre-acceptance: claimable while accept() is still admissible (block.timestamp <=
            // deadline); past the deadline, start == end → zero window.
        ...
        if (agreementScope & SCOPE_PENDING != 0 && _a.pendingTermsHash != bytes32(0)) {
            ...
            // Skip expired pending — {update} rejects past-deadline RCAUs, so a stale pending
            // offer can never be promoted to active and cannot be claimed on.
