...
import { IIssuanceAllocator } from "@graphprotocol/issuance/contracts/interfaces/IIssuanceAllocator.sol";
...
interface IRewardsManagerState {
    ...
    /// @return The address of the RewardsEligibilityOracle contract
...
contract IssuanceStateVerifier {
    ...
    /// @notice Asserts that RewardsManager has the expected RewardsEligibilityOracle address set
    /// @dev Reverts with ValueMismatch if the addresses don't match
    /// @param rewardsManager The RewardsManager contract to check
    /// @param expectedREO The expected RewardsEligibilityOracle address
    function assertRewardsEligibilityOracleSet(IRewardsManagerState rewardsManager, address expectedREO) external view {
        address current = rewardsManager.rewardsEligibilityOracle();
        if (current != expectedREO) revert ValueMismatch(expectedREO, current);
    }
