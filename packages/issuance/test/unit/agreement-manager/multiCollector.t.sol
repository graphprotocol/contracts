...
contract RecurringAgreementManagerMultiCollectorTest is RecurringAgreementManagerSharedTest {
    ...
    function test_MultiCollector_RequiredEscrowIsolation() public {
        ...
        vm.prank(operator);
        agreementManager.offerAgreement(rca1, address(recurringCollector));
        ...
        vm.prank(operator);
        agreementManager.offerAgreement(rca2, address(collector2));
        ...
        assertEq(agreementManager.getRequiredEscrow(address(collector2), indexer), maxClaim2);
    }
    ...
    function test_MultiCollector_SeparateEscrowAccounts() public {
        ...
        uint256 maxClaim1 = 1 ether * 3600 + 100 ether;
        token.mint(address(agreementManager), maxClaim1);
        vm.prank(operator);
        agreementManager.offerAgreement(rca1, address(recurringCollector));
        ...
        uint256 maxClaim2 = 2 ether * 7200 + 200 ether;
        token.mint(address(agreementManager), maxClaim2);
        vm.prank(operator);
        agreementManager.offerAgreement(rca2, address(collector2));
        ...
        assertEq(
            paymentsEscrow.escrowAccounts(address(agreementManager), address(collector2), indexer).balance,
            maxClaim2
        );
    }
