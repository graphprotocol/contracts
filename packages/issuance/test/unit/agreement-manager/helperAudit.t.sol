...
contract RecurringAgreementHelperAuditTest is RecurringAgreementManagerSharedTest {
    ...
    function test_AuditPairs_EmptyCollector() public view {
        IRecurringAgreementHelper.ProviderAudit[] memory pairs = agreementHelper.auditProviders(
            IAgreementCollector(address(recurringCollector))
        );
    ...
    function test_GetProviderAgreements_Paginated() public {
        ...
        // First page
        bytes16[] memory first = agreementHelper.getAgreements(
            IAgreementCollector(address(recurringCollector)),
            indexer,
            0,
            1
        ...
        // Second page
        bytes16[] memory second = agreementHelper.getAgreements(
            IAgreementCollector(address(recurringCollector)),
            indexer,
            1,
            1
        ...
        // Past end
        bytes16[] memory empty = agreementHelper.getAgreements(
            IAgreementCollector(address(recurringCollector)),
            indexer,
            2,
            1
        ...
        bytes16[] memory clamped = agreementHelper.getAgreements(
            IAgreementCollector(address(recurringCollector)),
            indexer,
            1,
            100
    ...
    function test_CheckPairStaleness_DetectsStaleAgreement() public {
        ...
        (IRecurringAgreementHelper.AgreementStaleness[] memory stale, bool escrowStale) = agreementHelper
            .checkStaleness(IAgreementCollector(address(recurringCollector)), indexer);
        ...
        // Now cached != live
        (stale, escrowStale) = agreementHelper.checkStaleness(
            IAgreementCollector(address(recurringCollector)),
            indexer
        );
        assertEq(stale.length, 1);
