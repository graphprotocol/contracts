...
import { ServiceAgreementManager } from "../../../contracts/agreement/ServiceAgreementManager.sol";
...
/// @notice Shared test setup for ServiceAgreementManager tests.
contract ServiceAgreementManagerSharedTest is Test {
    // -- Contracts --
    ...
    ServiceAgreementManager internal agreementManager;
    ...
    function setUp() public virtual {
        ...
        // Deploy ServiceAgreementManager behind proxy
        ServiceAgreementManager impl = new ServiceAgreementManager(
            address(token),
            address(paymentsEscrow),
            address(recurringCollector)
        );
        bytes memory initData = abi.encodeCall(ServiceAgreementManager.initialize, (governor));
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(impl),
            address(this), // proxy admin
            initData
        );
        agreementManager = ServiceAgreementManager(address(proxy));
        ...
        vm.label(address(agreementManager), "ServiceAgreementManager");
        vm.label(address(mockSubgraphService), "SubgraphService");
    }

    // -- Helpers --

    /// @notice Create a standard RCA with ServiceAgreementManager as payer
    function _makeRCA(
    ...
    function _offerAgreement(IRecurringCollector.RecurringCollectionAgreement memory rca) internal returns (bytes16) {
        // Fund ServiceAgreementManager with enough tokens
