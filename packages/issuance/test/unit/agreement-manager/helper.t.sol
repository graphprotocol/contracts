...
contract RecurringAgreementHelperTest is RecurringAgreementManagerSharedTest {
    ...
    function _setSimulatedAgreement(
        ...
        MockRecurringCollector.AgreementStorage memory simData;
        simData.dataService = dataService;
        simData.payer = _payer;
        simData.serviceProvider = _serviceProvider;
        simData.acceptedAt = uint64(block.timestamp);
        simData.updateNonce = 1;
        simData.state = REGISTERED | ACCEPTED;
        simData.activeTerms.endsAt = uint64(block.timestamp + 730 days);
        simData.activeTerms.maxInitialTokens = 200 ether;
        simData.activeTerms.maxOngoingTokensPerSecond = 2 ether;
        simData.activeTerms.minSecondsPerCollection = 60;
        simData.activeTerms.maxSecondsPerCollection = 7200;
        recurringCollector.setAgreement(agreementId, simData);
    }
    ...
    function test_ReconcileBatch_ClearsPendingUpdate() public {
        ...
        // Simulate: accepted with the update already applied
        _setSimulatedAgreement(agreementId, rca.dataService, rca.payer, rca.serviceProvider);
