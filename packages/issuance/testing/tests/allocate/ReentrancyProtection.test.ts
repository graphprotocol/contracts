...
describe('IssuanceAllocator - Reentrancy Protection', () => {
  ...
  describe('Reentrancy during distributeIssuance', () => {
    it('should allow target to call distributeIssuance during notification (legitimate use case)', async () => {
      ...
      await issuanceAllocator
        ...
        ['setTargetAllocation(address,uint256)'](await reentrantTarget.getAddress(), ethersLib.parseEther('50'))
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256)'](await reentrantTarget.getAddress(), ethersLib.parseEther('40'))
    })
  })
  ...
  describe('Reentrancy during setTargetAllocation', () => {
    const testCases = [
      {
        ...
        trigger: async (target: string) =>
          issuanceAllocator
            .connect(accounts.governor)
            ['setTargetAllocation(address,uint256)'](target, ethersLib.parseEther('40')),
      },
      {
        ...
        trigger: async (target: string) =>
          issuanceAllocator
            .connect(accounts.governor)
            ['setTargetAllocation(address,uint256,uint256)'](target, ethersLib.parseEther('40'), 0),
      },
      {
    ...
    testCases.forEach(({ name, action, trigger }) => {
      it(`should revert when target attempts to reenter setTargetAllocation (${name})`, async () => {
        ...
        await issuanceAllocator
          .connect(accounts.governor)
          ['setTargetAllocation(address,uint256)'](targetAddress, ethersLib.parseEther('30'))
    ...
    const testCases = [
      {
        ...
        trigger: async () =>
          issuanceAllocator.connect(accounts.governor).setIssuancePerBlock(ethersLib.parseEther('200')),
      },
      {
        name: '2 param variant',
        ...
        trigger: async () =>
          issuanceAllocator
            .connect(accounts.governor)
            ['setIssuancePerBlock(uint256,uint256)'](ethersLib.parseEther('200'), 0),
      },
    ]
  ...
  describe('Reentrancy during notifyTarget', () => {
    it('should revert when target attempts to reenter notifyTarget', async () => {
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256)'](await reentrantTarget.getAddress(), ethersLib.parseEther('25'))
    ...
    testCases.forEach(({ name, action }) => {
      it(`should revert when target attempts to reenter distributePendingIssuance (${name})`, async () => {
        ...
        await expect(
          issuanceAllocator
            .connect(accounts.governor)
            ['setTargetAllocation(address,uint256)'](targetAddress, ethersLib.parseEther('40')),
        ).to.be.revertedWithCustomError(issuanceAllocator, 'ReentrancyGuardReentrantCall')
      })
    })
  })
  ...
  describe('No reentrancy when disabled', () => {
    it('should work normally when reentrancy is not attempted', async () => {
      ...
      // Mine some blocks
      await ethers.provider.send('hardhat_mine', ['0x0A']) // Mine 10 blocks
