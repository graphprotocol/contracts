...
import { Constants, deployTestGraphToken, getTestAccounts } from '../common/fixtures'
import { deployAllocateSystem } from './fixtures'
import { TestConstants } from './testPatterns'

/**
 * Enhanced fixture for allocate-only system (excludes eligibility contracts)
 * Use this for allocate tests to ensure clean separation from eligibility
 */
export async function setupOptimizedAllocateSystem(customOptions: any = {}) {
  const accounts = await getTestAccounts()

  const options = {
    issuancePerBlock: Constants.DEFAULT_ISSUANCE_PER_BLOCK,
  ...
  // Deploy allocate-only system (no eligibility contracts)
  const { graphToken, issuanceAllocator, target1, target2 } = await deployAllocateSystem(
    accounts,
    options.issuancePerBlock,
  )
...
/**
 * Lightweight fixture for testing single allocate contracts
 */
export async function setupSingleContract(contractType: 'issuanceAllocator' | 'directAllocation') {
  const accounts = await getTestAccounts()
  ...
  switch (contractType) {
    case 'issuanceAllocator': {
      const { deployIssuanceAllocator } = await import('./fixtures')
      contract = await deployIssuanceAllocator(
        graphTokenAddress,
        accounts.governor,
    ...
    case 'directAllocation': {
      const { deployDirectAllocation } = await import('./fixtures')
      contract = await deployDirectAllocation(graphTokenAddress, accounts.governor)
      break
    }
    default:
...
export const TestData = {
  ...
  issuanceRates: {
    low: ethersLib.parseEther('10'),
    medium: ethersLib.parseEther('100'),
    high: ethersLib.parseEther('1000'),
  },
...
export class OptimizedFixtures {
  ...
  async setupDirectAllocationSuite() {
    ...
    const graphTokenAddress = await graphToken.getAddress()
    const { deployDirectAllocation } = await import('./fixtures')
    const directAllocation = await deployDirectAllocation(graphTokenAddress, this.accounts.governor)
    const directAllocationAddress = await directAllocation.getAddress()

    const { GraphTokenHelper } = require('../common/graphTokenHelper')
