...
describe('DirectAllocation - Optimized & Consolidated', () => {
  ...
  describe('Constructor Validation', () => {
    it('should revert when constructed with zero GraphToken address', async () => {
      ...
      await expect(DirectAllocationFactory.deploy(ethersLib.ZeroAddress)).to.be.revertedWithCustomError(
  ...
  describe('Initialization', () => {
    ...
    it('should revert when initialized with zero governor address', async () => {
      ...
      const DirectAllocationFactory = await ethers.getContractFactory('DirectAllocation')
      const implementation = await DirectAllocationFactory.deploy(graphTokenAddress)
      await implementation.waitForDeployment()

      // Encode initializer call with zero address
      const initData = DirectAllocationFactory.interface.encodeFunctionData('initialize', [ethersLib.ZeroAddress])

      // Load TransparentUpgradeableProxy artifact from @openzeppelin/contracts
      const proxyArtifactPath =
        require.resolve('@openzeppelin/contracts/build/contracts/TransparentUpgradeableProxy.json')
      const ProxyArtifact = JSON.parse(fs.readFileSync(proxyArtifactPath, 'utf8'))

      // Create proxy factory from artifact
      const ProxyFactory = new ethers.ContractFactory(ProxyArtifact.abi, ProxyArtifact.bytecode, accounts.governor)
      await expect(
        ProxyFactory.deploy(await implementation.getAddress(), accounts.governor.address, initData),
  ...
  describe('Token Management', () => {
    it('should handle token operations with proper access control and validation', async () => {
      ...
      await graphTokenHelper.mint(await directAllocation.getAddress(), ethersLib.parseEther('1000'))
      ...
      await expect(
        directAllocation.connect(accounts.nonGovernor).sendTokens(accounts.user.address, ethersLib.parseEther('100')),
      ...
      await expect(
        directAllocation.connect(accounts.operator).sendTokens(ethersLib.ZeroAddress, ethersLib.parseEther('100')),
    ...
    it('should handle insufficient balance and pause states correctly', async () => {
      ...
      await directAllocation.connect(accounts.governor).grantRole(OPERATOR_ROLE, accounts.operator.address)
      await expect(
        directAllocation.connect(accounts.operator).sendTokens(accounts.user.address, ethersLib.parseEther('100')),
