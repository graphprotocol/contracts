...
describe('IssuanceAllocator', () => {
  ...
  before(async () => {
    ...
    issuancePerBlock = ethersLib.parseEther('100') // Default issuance per block
  ...
  beforeEach(async () => {
    if (!accounts) {
      accounts = await getTestAccounts()
      issuancePerBlock = ethersLib.parseEther('100')
    }
  ...
  describe('ERC-165 Interface Checking', () => {
    ...
    it('should revert when adding EOA targets (no contract code)', async () => {
      ...
      // This is a low-level EVM revert, not a custom error, so we verify it reverts
      let reverted = false
      try {
        await issuanceAllocator
          .connect(accounts.governor)
  ...
  describe('Target Management', () => {
    ...
    it('should validate setTargetAllocation parameters and constraints', async () => {
      ...
      // This is a low-level EVM revert, not a custom error
      let reverted = false
      try {
        await issuanceAllocator
          .connect(accounts.governor)
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256,uint256)'](addresses.target1, ethersLib.parseEther('60'), 0)

      // Try to set allocation for target2 to 50%, which would exceed 100% (60% + 50% > 100%)
      await expectCustomError(
        issuanceAllocator
          .connect(accounts.governor)
          ['setTargetAllocation(address,uint256,uint256)'](addresses.target2, ethersLib.parseEther('50'), 0),
        issuanceAllocator,
  ...
  describe('Issuance Rate Management', () => {
    it('should update issuance rate correctly', async () => {
      ...
      const newIssuancePerBlock = ethersLib.parseEther('200')
    ...
    it('should revert when decreasing issuance rate with insufficient unallocated budget', async () => {
      ...
      await issuanceAllocator.connect(accounts.governor).setIssuancePerBlock(ethersLib.parseEther('1000'))
      ...
      await expect(
        issuanceAllocator.connect(accounts.governor).setIssuancePerBlock(ethersLib.parseEther('900')),
    ...
    it('should revert when notifying a non-existent target (EOA)', async () => {
      ...
      let reverted = false
      try {
        await issuanceAllocator.connect(accounts.governor).notifyTarget(accounts.nonGovernor.address)
      } catch {
        reverted = true
      }
      expect(reverted).to.be.true
    ...
    it('should return false when setTargetAllocation called with force=false and issuance distribution is behind', async () => {
      ...
      await issuanceAllocator.connect(accounts.governor).setIssuancePerBlock(ethersLib.parseEther('100'))
      await issuanceAllocator.connect(accounts.governor).distributeIssuance()
      ...
      const result = await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ].staticCall(await target1.getAddress(), ethersLib.parseEther('30'), 0, currentBlock)
  ...
  describe('Return Values', () => {
    ...
    describe('setIssuancePerBlock', () => {
      it('should return appropriate values based on conditions', async () => {
        ...
        const newRate = ethersLib.parseEther('200')
        ...
        const differentRate = ethersLib.parseEther('2000')
  ...
  describe('getTargetIssuancePerBlock', () => {
    it('should return correct issuance for different target configurations', async () => {
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256,uint256)'](addresses.target1, 0, ethersLib.parseEther('30'))

      const expectedSelfIssuance = ethersLib.parseEther('30')
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256,uint256)'](addresses.target1, ethersLib.parseEther('40'), 0)

      const expectedAllocatorIssuance = ethersLib.parseEther('40')
      result = await issuanceAllocator.getTargetIssuancePerBlock(addresses.target1)
    ...
    it('should not revert when contract is paused and blockAppliedTo indicates pause state', async () => {
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256,uint256)'](addresses.target1, 0, ethersLib.parseEther('30')) // 30%, self-minter
      ...
      // const issuancePerBlock = await issuanceAllocator.getIssuancePerBlock()
      // const PPM = 1_000_000
      const expectedIssuance = ethersLib.parseEther('30')
    ...
    it('should show blockAppliedTo updates after distribution', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('50'), 0, 0) // 50%, allocator-minter
      ...
      // const issuancePerBlock = await issuanceAllocator.getIssuancePerBlock()
      // const PPM = 1_000_000
      const expectedIssuance = ethersLib.parseEther('50')
  ...
  describe('Notification Behavior When Paused', () => {
    ...
    it('should notify targets of issuance rate changes even when paused', async () => {
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setIssuancePerBlock(uint256,uint256)'](ethersLib.parseEther('200'), lastDistributionBlock)

      // Verify that the rate change was applied
      expect(await issuanceAllocator.getIssuancePerBlock()).to.equal(ethersLib.parseEther('200'))
    })
    ...
    it('should not notify targets when no actual change occurs', async () => {
      ...
      expect(await issuanceAllocator.getIssuancePerBlock()).to.equal(ethersLib.parseEther('100'))
    })
  })
  ...
  describe('Pending Issuance Distribution', () => {
    ...
    it('should not emit SelfMintingOffsetReconciled when offset unchanged', async () => {
      ...
      await issuanceAllocator
        .connect(accounts.governor)
        ['setTargetAllocation(address,uint256,uint256)'](await target1.getAddress(), ethersLib.parseEther('50'), 0)
    ...
    it('should handle proportional distribution when available < allocatedTotal', async () => {
      ...
      await issuanceAllocator.connect(accounts.governor)['setTargetAllocation(address,uint256,uint256,uint256)'](
        ...
        ethersLib.parseEther('400'), // 400 ether per block allocator-minting
        0,
        0,
      )
      await issuanceAllocator.connect(accounts.governor)['setTargetAllocation(address,uint256,uint256,uint256)'](
        ...
        ethersLib.parseEther('400'), // 400 ether per block allocator-minting
        ethersLib.parseEther('150'), // 150 ether per block self-minting
        0,
      )
      ...
      expect(distributed1).to.be.closeTo(distributed2, ethersLib.parseEther('1'))
    })
    ...
    it('should revert when non-governor calls distributePendingIssuance()', async () => {
      ...
      // Try to call distributePendingIssuance() as non-governor - should revert
      await expect(
        issuanceAllocator.connect(accounts.user)['distributePendingIssuance()'](),
      ).to.be.revertedWithCustomError(issuanceAllocator, 'AccessControlUnauthorizedAccount')
    })
    ...
    it('should revert when non-governor calls distributePendingIssuance(uint256)', async () => {
      ...
      // Try to call distributePendingIssuance(uint256) as non-governor - should revert
      await expect(
        issuanceAllocator.connect(accounts.user)['distributePendingIssuance(uint256)'](blockNumber),
      ).to.be.revertedWithCustomError(issuanceAllocator, 'AccessControlUnauthorizedAccount')
    })
    ...
    it('should revert when toBlockNumber < lastDistributionBlock', async () => {
      ...
      await expect(
        issuanceAllocator.connect(accounts.governor)['distributePendingIssuance(uint256)'](pastBlock),
      ).to.be.revertedWithCustomError(issuanceAllocator, 'ToBlockOutOfRange')
    })
    ...
    it('should handle exact allocation with zero remainder to default', async () => {
      ...
      await issuanceAllocator.connect(accounts.governor)['setTargetAllocation(address,uint256,uint256,uint256)'](
        await target1.getAddress(),
        ethersLib.parseEther('800'), // 800 ether per block allocator-minting
        ethersLib.parseEther('200'), // 200 ether per block self-minting
        0,
      )
      ...
      // remainder = 0 ✓
      const finalBalance = await (graphToken as any).balanceOf(await target1.getAddress())
      const expectedDistribution = ethersLib.parseEther('800') * BigInt(blocksDist)
    ...
    it('should handle proportional distribution with target having zero allocator rate', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('400'), 0, 0)
      await issuanceAllocator.connect(accounts.governor)['setTargetAllocation(address,uint256,uint256,uint256)'](
        await target2.getAddress(),
  ...
  describe('Pause/Unpause Edge Cases', () => {
    ...
    it('should handle unpause → mine blocks → pause without distributeIssuance', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('30'), 0, 0) // 30 tokens/block allocator
      await issuanceAllocator
        .connect(accounts.governor)
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target2.getAddress(), 0, ethersLib.parseEther('20'), 0) // 20 tokens/block self
      ...
      const selfMintingRate = ethersLib.parseEther('20') // 20% of 100 = 20 tokens/block
      const expectedAccumulation = selfMintingRate * blocksSinceInitial
      expect(distState1.selfMintingOffset).to.be.closeTo(expectedAccumulation, ethersLib.parseEther('1'))
      ...
      const totalIssuance = ethersLib.parseEther('100') * totalBlocks
      ...
      const expectedAllocatorDistribution =
        ((totalIssuance - expectedAccumulation) * ethersLib.parseEther('30')) / ethersLib.parseEther('80')
      ...
      expect(totalDistributed).to.be.closeTo(expectedAllocatorDistribution, ethersLib.parseEther('25'))
    })
    ...
    it('should use getDistributionState to query distribution state efficiently', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), 0, ethersLib.parseEther('50'), 0) // 50 tokens/block self
    ...
    it('should correctly emit IssuanceSelfMintAllowance events across pause/unpause cycles', async () => {
      ...
      const expectedAmount = ethersLib.parseEther('50') * blocksInRange
      expect(decodedEvent.amount).to.be.closeTo(expectedAmount, ethersLib.parseEther('1'))
    })
    ...
    it('should continue accumulating through unpaused periods when accumulated balance exists', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('30'), ethersLib.parseEther('20'), 0)
      ...
      const expectedAccumulation1 = ethersLib.parseEther('20') * BigInt(pausedBlocks1)
      ...
      const expectedAccumulation = ethersLib.parseEther('20') * BigInt(blocksAccumulated)
    ...
    it('should correctly handle partial distribution when toBlockNumber < block.number', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('30'), 0, 0)
      await issuanceAllocator
        .connect(accounts.governor)
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
      ...
      const totalIssuance = ethersLib.parseEther('100') * totalBlocks
      const totalSelfMinting = ethersLib.parseEther('20') * totalBlocks
      ...
      const expectedForTarget1 = (availableForAllocator * ethersLib.parseEther('30')) / ethersLib.parseEther('80')
      ...
      expect(balance1).to.be.closeTo(expectedForTarget1, ethersLib.parseEther('100'))
    })
    ...
    it('should correctly handle accumulated self-minting that exceeds period budget', async () => {
      ...
      await issuanceAllocator
        ...
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target1.getAddress(), ethersLib.parseEther('20'), 0, 0)
      await issuanceAllocator
        .connect(accounts.governor)
        [
          'setTargetAllocation(address,uint256,uint256,uint256)'
        ](await target2.getAddress(), 0, ethersLib.parseEther('80'), 0)
      ...
      const _expectedAccumulated = ethersLib.parseEther('80') * BigInt(blocksAccumulated)
      ...
      const totalExpectedAccumulated = ethersLib.parseEther('80') * BigInt(totalBlocksAccumulated)
      ...
      const totalForPeriod = ethersLib.parseEther('100') * BigInt(blocksDistributed)
      ...
      expect(stateAfter.selfMintingOffset).to.be.closeTo(expectedRemaining, ethersLib.parseEther('10'))
    })
  })
})
