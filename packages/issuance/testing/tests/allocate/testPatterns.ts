...
export function shouldEnforceGovernorRole<T>(
  ...
  return function () {
    ...
    it(`should allow governor to call ${methodName}`, async function () {
      ...
      await (contract as any).connect(testAccounts.governor)[methodName](...methodArgs)
    })
  }
}
      ...
      describe(`${method.name} access control`, () => {
        ...
        allowedRoles.forEach((role) => {
          ...
          it(`should allow ${roleName} to call ${method.name}`, async function () {
            const contract = contractGetter()
            // In HH v3, just await the call - if it reverts, the test fails
            await (contract as any).connect(account)[method.name](...method.args)
          })
        })
      })
    })
  ...
  return function () {
    describe('Pausability', () => {
      it('should allow PAUSE_ROLE to pause and unpause', async function () {
        ...
        // Should be able to pause - in HH v3, just await the call
        await (contract as any).connect(accounts.operator).pause()
        expect(await (contract as any).paused()).to.be.true

        // Should be able to unpause - in HH v3, just await the call
        await (contract as any).connect(accounts.operator).unpause()
        expect(await (contract as any).paused()).to.be.false
      })
        ...
        describe(`${roleConfig.roleName} management`, () => {
          grantableBy.forEach((granterRole) => {
            ...
            it(`should allow ${granterRole} to revoke ${roleConfig.roleName}`, async function () {
              ...
              // Then revoke it - in HH v3, just await the call
              await (contract as any).connect(granter).revokeRole(roleConfig.role, accounts.user.address)
