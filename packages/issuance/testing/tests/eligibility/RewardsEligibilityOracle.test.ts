...
import type { RewardsEligibilityOracle } from '@graphprotocol/issuance/types'
...
describe('RewardsEligibilityOracle', () => {
  ...
  describe('Construction', () => {
    it('should revert when constructed with zero GraphToken address', async () => {
      ...
      await expect(RewardsEligibilityOracleFactory.deploy(ethersLib.ZeroAddress)).to.be.revertedWithCustomError(
    ...
    it('should revert when initialized with zero governor address', async () => {
      ...
      const RewardsEligibilityOracleFactory = await ethers.getContractFactory('RewardsEligibilityOracle')
      const implementation = await RewardsEligibilityOracleFactory.deploy(graphTokenAddress)
      await implementation.waitForDeployment()

      // Encode initializer call with zero address
      const initData = RewardsEligibilityOracleFactory.interface.encodeFunctionData('initialize', [
  ...
  describe('Indexer Management', () => {
    ...
    it('should return correct count for various renewIndexerEligibility scenarios', async () => {
      ...
      const withZeroAddresses = [accounts.indexer1.address, ethersLib.ZeroAddress, accounts.indexer2.address]
  ...
  describe('View Functions', () => {
    ...
    it('should return true for all indexers when oracle update timeout is exceeded', async function () {
      ...
      // Advance time beyond the timeout using direct provider calls
      const ethers = await getEthers()
    ...
    it('should return false for indexer after eligibility period expires', async function () {
      ...
      // Advance time beyond eligibility period using direct provider calls
      const ethers = await getEthers()
      await ethers.provider.send('evm_increaseTime', [120]) // 2 minutes
      await ethers.provider.send('evm_mine', []) // Mine a block to apply the new timestamp
