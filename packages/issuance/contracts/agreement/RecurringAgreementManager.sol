    ...
    IProviderEligibility
{
    ...
    struct RecurringAgreementManagerStorage {
        ...
        /// @notice Minimum thaw fraction: thaws below sumMaxNextClaim * minThawFraction / 256
        /// per (collector, provider) pair are skipped as operationally insignificant.
        /// Governance-configured.
    ...
    function _updateEscrow(RecurringAgreementManagerStorage storage $, address collector, address provider) private {
        ...
        uint256 minThawAmount = ($.sumMaxNextClaim[collector][provider] * uint256($.minThawFraction)) / 256;
        ...
        // 3. Skip thaw if excess above max is below thaw threshold.
        uint256 excess = max < escrowed ? escrowed - max : 0;
        uint256 thawTarget = (escrowed < min)
            ? (min < account.balance ? account.balance - min : 0)
            : (minThawAmount <= excess ? account.balance - max : account.tokensThawing);
        ...
        _withdrawAndRebalance(collector, provider, account, min, max, minThawAmount);
        _setEscrowSnap($, collector, provider);
    }
     ...
     * @param minThawAmount Dust threshold — skip thaw if excess is not above this
     */
    function _withdrawAndRebalance(
        ...
        if (account.tokensThawing == 0) {
            if (max < account.balance && minThawAmount <= account.balance - max)
