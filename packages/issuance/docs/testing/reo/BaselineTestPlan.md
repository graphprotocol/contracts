...
{
  subgraphDeployments(
    where: { deniedAt: 0, signalledTokens_not: 0, indexingRewardAmount_not: 0 }
  ) {
    ipfsHash
    stakedTokens
    signalledTokens
...
{
  allocations(
    where: { indexer_: { id: "INDEXER_ADDRESS" }, status: "Active" }
  ) {
    id
    allocatedTokens
    createdAtEpoch
...
{
  allocations(
    where: { indexer_: { id: "INDEXER_ADDRESS" }, status: "Closed" }
  ) {
    id
    queryFeesCollected
    closedAtEpoch
...
_Extracted from Horizon upgrade test plans. Source: [notion/](notion/) directory._
