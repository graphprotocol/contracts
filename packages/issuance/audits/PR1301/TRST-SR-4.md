...
The try/catch is intentional — provider liveness takes priority over payer state consistency. The snap-refresh fix (TRST-H-3) ensures the next successful `_updateEscrow` call self-corrects any divergence. Permissionless `reconcileAgreement` and `reconcileCollectorProvider` provide external recovery paths.

Both catch blocks now emit `PayerCallbackFailed(agreementId, payer, stage)` (with a `PayerCallbackStage` enum distinguishing `BeforeCollection` / `AfterCollection`), giving off-chain monitoring a signal to detect repeated failures and trigger reconciliation before divergence compounds.
