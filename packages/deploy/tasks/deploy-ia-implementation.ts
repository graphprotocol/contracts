...
/**
 * Deploy new IssuanceAllocator implementation with automated orchestration
 *
 * This task:
 * 1. Deploys new IssuanceAllocator implementation contract
 ...
 *
 * Usage:
 *   npx hardhat issuance:deploy-ia-implementation --network arbitrumOne
 *   npx hardhat issuance:deploy-ia-implementation --network arbitrumSepolia
 */
task(
  'issuance:deploy-ia-implementation',
  'Deploy new IssuanceAllocator implementation and prepare for governance upgrade',
)
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    // Step 1: Deploy new IssuanceAllocator implementation
    console.log('\n📦 Step 1: Deploying new IssuanceAllocator implementation...')

    const IssuanceAllocatorImplementationModule = await import(
      '@graphprotocol/issuance/deploy/ignition/modules/IssuanceAllocatorImplementation'
    )
    const { IssuanceAllocatorImplementation } = await ignition.deploy(IssuanceAllocatorImplementationModule.default)

    const implAddress = await IssuanceAllocatorImplementation.getAddress()
    const deployTx = IssuanceAllocatorImplementation.deploymentTransaction()
    ...
    addressBook.setPendingImplementation('IssuanceAllocator', implAddress, {
    ...
    const result = await buildIssuanceContractUpgradeTxs(
      hre,
      {
        contractName: 'IssuanceAllocator',
    ...
    console.log(
      `      npx hardhat issuance:sync-pending-implementation --contract IssuanceAllocator --network ${hre.network.name}`,
    )
