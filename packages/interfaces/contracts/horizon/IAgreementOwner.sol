...
import { IAgreementStateChangeCallback } from "./IAgreementStateChangeCallback.sol";
 ...
 *
 * Lifecycle callback (inherited from {IAgreementStateChangeCallback}):
 * - {afterAgreementStateChange}: non-reverting notification on agreement lifecycle events.
 *   Only fires for events NOT initiated by the payer (the collector skips callbacks to
 *   `msg.sender`). Implementations should filter by state flags and ignore unrecognised
 *   combinations.
 *
 * @custom:security-contact Please email security+contracts@thegraph.com if you find any
 * bugs. We may have an active bug bounty program.
 */
interface IAgreementOwner is IAgreementStateChangeCallback {
    /**
