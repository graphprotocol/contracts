...
// Used in two contexts: offer-scoped (returned by offer()) and agreement-scoped
// (returned by getAgreementDetails()). See AgreementDetails.state NatSpec.
...
/// @dev Nothing to collect in current state
...
interface IAgreementCollector is IPaymentsCollector {
     ...
     *
     * The `state` field is context-dependent:
     * - When returned by {offer}: offer-scoped — describes the offer just created
     *   (e.g. REGISTERED, REGISTERED | UPDATE). Does not include agreement-level
     *   flags like ACCEPTED.
     * - When returned by {getAgreementDetails}: agreement-scoped — describes the
     *   current agreement state (e.g. ACCEPTED, ACCEPTED | UPDATE).
     *
     ...
     * @param state State flags (offer-scoped from {offer}, agreement-scoped from {getAgreementDetails})
     */
     ...
     * @dev The returned {AgreementDetails}.state is offer-scoped: it reflects the
     * offer that was just stored (REGISTERED, REGISTERED | UPDATE), not the full
     * agreement lifecycle state. Use {getAgreementDetails} for agreement-scoped flags.
