...
interface IAgreementCollector is IPaymentsCollector {
     ...
     * context of its agreement. Flags that depend on the version (REGISTERED,
     * ACCEPTED, UPDATE) are set only when they apply to that specific version;
     * flags that describe the agreement-wide collectability of that version
     * (NOTICE_GIVEN, BY_PAYER, BY_PROVIDER, SETTLED) reflect the current
     * agreement state. Identical semantics whether returned by {offer} or
     * {getAgreementDetails} — the returned flags always describe the queried
     * version.
     *
     ...
     * version-specific flags (REGISTERED, ACCEPTED, UPDATE) are set when they
     * apply to the offered version; agreement-wide flags (NOTICE_GIVEN, BY_*,
     * SETTLED) reflect current agreement state.
