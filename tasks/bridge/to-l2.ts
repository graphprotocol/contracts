...
import {
  sendSubgraphToL2,
  finishSendSubgraphToL2,
  claimCuratorBalanceOnL2,
} from '../../cli/commands/bridge/gns'
...
export const TASK_CLAIM_CURATOR_BALANCE_L2 = 'bridge:claim-curator-balance-on-l2'
...
task(TASK_CLAIM_CURATOR_BALANCE_L2, 'Claim curator balance from L1 to L2')
  .addParam('subgraphId', 'Supgraph ID')
  .addFlag('disableSecureAccounts', 'Disable secure accounts on GRE')
  .addOptionalParam('sender', 'Address of the sender. L1 deployer if empty.')
  .addOptionalParam(
    'l2Beneficiary',
    'Address to own the signal on L2. Same as L1 address if empty.',
  )
  .addOptionalParam('addressBook', cliOpts.addressBook.description)
  .addOptionalParam(
    'arbitrumAddressBook',
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    await claimCuratorBalanceOnL2(await loadEnv(taskArgs, wallet), taskArgs)
