...
contract('GNS', accounts => {
  ...
  const hashedSubdomain = web3.utils.soliditySha3(web3.utils.soliditySha3(subdomainName), topLevelDomainHash) // NOTE: There is a bug with web3.utils.keccak256() when using multiple inputs. soliditySha3() must be used
  ...
  it('...should allow a user to register a domain. And then prevent another user from being able to', async () => {
    ...
    await expectRevert(deployedGNS.registerDomain(topLevelDomainName, { from: accounts[3] }), 'Domain is already owned.')
  ...
  it('...should allow subgraph metadata to be updated', async () => {
    ...
    await expectRevert(deployedGNS.changeSubgraphMetadata(ipfsHash, hashedSubdomain, { from: accounts[3] }), 'Only domain owner can call')
  })

  it('...should allow a user to transfer a domain', async () => {
    const { logs } = await deployedGNS.transferDomainOwnership(hashedSubdomain, accounts[2], { from: accounts[1] })
  ...
  it('...should allow a domain and subgraphID to be deleted', async () => {
    await expectRevert(deployedGNS.deleteSubdomain(hashedSubdomain, { from: accounts[3] }), 'Only domain owner can call')

    const { logs } = await deployedGNS.deleteSubdomain(hashedSubdomain, { from: accounts[2] })
  ...
  it('...should allow account metadata event to be emitted  ', async () => {
    ...
    const { logs } = await deployedGNS.changeAccountMetadata(accountIPFSHash, { from: accounts[2] })
