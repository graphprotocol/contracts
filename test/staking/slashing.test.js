...
contract('Staking (Slashing)', ([
  ...
  const minimumCurationStakingAmount = 100,
    ...
    chainId = 1,
    domainTypeHash = web3.utils.sha3("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"),
  ...
  async function createDisputeDataWithSignedAttestation() {
    ...
    // required bytes: 1 + 32 + 229 = 262
    const tokensReceivedHexData = '0x'
      + '02' // TokenReceiptAction.dispute (1 byte)
      + subgraphIdHex // Subgraph ID without `0x` (32 bytes)
      + encodedAttestation.substring(2) // Hex encoded attestation w/o `0x` (< 229 bytes)
      + signedAttestation.substring(2) // IEP712 domain separator signed attestation (< 229 bytes)
    ...
    //   expectedEncodedAttestationByteLength: '< 229',
    ...
    //   expectedSignedAttestationByteLength: '< 229',
    ...
    //   subgraphIdByteLength: web3.utils.hexToBytes(String('0x' + subgraphIdHex)).length,

    //   tokensReceivedHexData,
    //   tokensReceivedHexDataLength: tokensReceivedHexData.length,
    //   tokensReceivedByteDataLength: data.length,
    //   expectedTokensReceivedByteDataLength: 262,

    //   dataSentToTokensReceived: String("[" + data.toString() + "]"),
    //   PASS: data.length === 262, // data.length should be 1 + 32 + 229 = 262
    //   discrepancy: data.length - 262
    // })
    return data
  }
})
