...
contract(
  'Staking (Bonding Curve)',
  ([deploymentAddress, daoContract, ...accounts]) => {
    before(async () => {
      resetBondingParams()
    ...
    describe('purchaseReturn formula', () => {
      it('...should calculate shares expected from `purchaseReturn` formula', async () => {
        ...
        assert(
          typeof expectedShares === 'number',
          'Purchase Return is a number.',
        )
      })
...
async function testBondingCurve(
  ...
  for (let r = 0; r < reserveRatios.length; r++) {
    ...
    for (let p = 0; p < _purchaseCount; p++) {
      const shares = await computeStakeToShares(
        _purchaseAmount,
        reserveRatios[r],
        purchaseReturn,
      )
