...
describe('GNS', () => {
  ...
  let newSubgraph2: PublishSubgraph
  ...
  const curationPoolTokensAndSignal = async (
    subgraphDeploymentID: string,
  ...
  const publishNewVersion = async (
    ...
    // Get curation pool data from previous deployment
    const [tokensBefore, vSignalBefore] = await curationPoolTokensAndSignal(
      beforeSubgraph.subgraphDeploymentID,
    )

    // Get curation pool data from current deployment
    const [tokensAfter, vSignalAfter] = await curationPoolTokensAndSignal(
      newSubgraph.subgraphDeploymentID,
    )
  ...
  const deprecateSubgraph = async (account: Account, subgraphID: string) => {
    ...
    let tokens
    const beforeSubgraph = await gns.subgraphs(subgraphID)
    const [oldTokens] = await curationPoolTokensAndSignal(idOld)
    const [newTokens] = await curationPoolTokensAndSignal(idNew)
    tokens = oldTokens.add(newTokens)
    ...
    // Check args
    await expect(tx).emit(gns, 'SubgraphDeprecated').withArgs(subgraphID, tokens)
    ...
    expect(afterVSignalNew).eq(BigNumber.from(0))
    ...
    expect(afterSubgraph.withdrawableGRT).eq(tokens)
  ...
  const upgradeNameSignal = async (
    ...
    const beforeTokensVSigOldCuration = await curationPoolTokensAndSignal(subgraph0.subgraphDeploymentID)
    ...
    const [afterTokensOldCuration, vSigAfterOldCuration] = await curationPoolTokensAndSignal(
    ...
    const [afterTokensNewCurve, vSigAfterNewCurve] = await curationPoolTokensAndSignal(
      newSubgraphDeplyomentID,
    )
  ...
  */
  ...
  const burnSignal = async (
    account: Account,
    subgraphID: string,
    expected: ExpectedMintSignalValues = {},
  ): Promise<ContractTransaction> => {
    const tokensExpectedOld = expected.tokensOld || 0
    const vSignalExpectedOld = expected.vSignalOld || 0
    const vSignalExpectedNew = expected.vSignalNew || 0
    const tokensExpectedNew = expected.tokensNew || 0
    const nSignalExpected = expected.nSignal || 0
    const vSignalExpected = expected.vSignal || 0
    const vSignalEmit = expected.vSignalEmit || 9746794344808963906n
    const tokensEmit = expected.tokensEmit || 9500000000000000000000n
    ...
    await expect(tx)
      ...
      .withArgs(
        ...
        account.address,
        beforeUsersNSignal,
        BigNumber.from(vSignalEmit),
        BigNumber.from(tokensEmit),
      )
    ...
    // Curation Pool data after transaction
    const [afterTokensOld, afterVSignalOld] = await curationPoolTokensAndSignal(idOld)
    const [afterTokensNew, afterVSignalNew] = await curationPoolTokensAndSignal(idNew)
  ...
  describe('Publishing names and versions', function () {
    ...
    describe('publishNewVersion', async function () {
      ...
      context('when no current signal', async function () {
        it('should publish a new version on an existing subgraph', async function () {
          const emptySignalSubgraph = await publishNewSubgraph(me, buildSubgraph())
          await publishNewVersion(me, emptySignalSubgraph.id, newSubgraph1, 0n, 0n, 0n, 0n)
        })
      })
    })
    ...
    describe('finalizeSubgraphUpgrade', async function () {
      context('when new version exists', async function () {
        ...
        it('should finalize version', async function () {
          ...
          const nSignalExpected = 9746794344808963906n
    ...
    describe('multiple minting', async function () {
      it('should mint less signal every time due to the bonding curve', async function () {
        const tokensToDepositMany = [
          toGRT('10000'), // should mint if we start with number above minimum deposit
          toGRT('10000'), // every time it should mint less GCS due to bonding curve...
          toGRT('10000'),
          toGRT('10000'),
          toGRT('20000'),
          toGRT('20000'),
          toGRT('123'),
          toGRT('1'), // should mint minimum deposit including tax
        ]
        const subgraph = await publishNewSubgraph(me, newSubgraph0)
  ...
  describe('Subgraph lifecycles scenarios', function () {
    ...
    beforeEach(async () => {
      subgraphID = buildSubgraphID(me.address, await gns.nextAccountSeqID(me.address))
